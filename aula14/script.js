...
form.addEventListener('submit', function(ev) {
    ...
    const numInputRows = document.querySelectorAll('.inputRow').length

    let technologies = []
    for(let i = 0; i < numInputRows; i++) {
        const techName = document.querySelector(`#inputRow-${i} input[name="techName"]`).value
        const techExp = document.querySelector(`#inputRow-${i} input[name="techExp-${i}"]:checked`).value
    ...
    fullNameInput.value = ''
    const stackInputs = document.getElementById('stackInputs')
    while (stackInputs.firstChild) {
        stackInputs.removeChild(stackInputs.firstChild)
    }
...
/*form.addEventListener('submit', function(ev) {
    ev.preventDefault()
    const fullNameInput = document.getElementById('fullName')
    const inputRows = document.querySelectorAll('.inputRow').length

    let technologies = []
    inputRows.forEach(function (row) {
        const techName = document.querySelector(`#${row.id} input[name="techName"]`).value
        const techExp = document.querySelector(`#${row.id} input[name="techName"]:checked`).value
        technologies.push({name: techName, exp: techExp})
    })
